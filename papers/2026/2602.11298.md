...
We address this challenge by introducing _Voxtral Realtime_, a 4B parameter natively streaming ASR model that supports 13 languages. Concretely, our contributions are:
...
1.  1.  Encoder warm-up (5% of training): The decoder is frozen and only the encoder and adapter are trained. This prevents the randomly initialized encoder from destabilizing the pre-trained decoder representations before it has learned to produce useful audio embeddings.
2.  2.  End-to-end (95% of training): The full model is trained jointly.
