...
1.  1.  Log-mel spectrograms were computed with 80 mel filters.
2.  2.  Mel-frequency cepstral coefficients were computed with 13 resulting values.
3.  3.  A total of 68 three-dimensional facial keypoints were provided by the dataset \[15\]. They were extracted using OpenFace \[6\].
4.  4.  Word2Vec vectors were computed using the publicly available Word2Vec model from Google and the Gensim python library \[39\]. Each vector is of length 300.
5.  5.  Doc2Vec vectors were also computed using Gensim \[39\]. Each vector is of length 300.
6.  6.  Universal sentence embeddings were computed using Tensorflow \[1\] from the publicly available release. Each vector is of length 512.
7.  7.  The LSTM consists of 10 layers with 128 hidden units and is also optimized with the same batch size, optimizer, etc. as stated in Appendix A.2.1.
8.  8.  Our causal CNN model is the same as the one outlined in Appendix A.2.1.
