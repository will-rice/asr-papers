...
arxiv_id: "2110.05241"
title:
  Streaming Transformer Transducer Based Speech Recognition Using Non-Causal
  Convolution
authors:
  - Yangyang Shi
  - Chunyang Wu
...
where $`\mathbf{K}_{L,i}^{n}`$ and $`\mathbf{V}_{L,i}^{n}`$ are the _key_ and _value_ copies from previous blocks. $`\mathbf{Z}_{C,i}^{n}`$ and $`\mathbf{Z}_{R,i}^{n}`$ are the attention output for $`\mathbf{C}_{i}^{n}`$ and $`\mathbf{R}_{i}^{n}`$ respectively; $`\mathbf{s}_{i}^{n}`$ is the mean of center block $`\mathbf{C}_{i}^{n}`$; $`{Attn}\hspace{0pt}(\mathbf{q},\mathbf{k},\mathbf{v})`$ is the attention operation defined in \[5\] with $`\mathbf{q}`$ , $`\mathbf{k}`$ and $`\mathbf{v}`$ being the query, key and value, respectively. $`U`$ specifies the number of slots in augmented memory; the most recent slots are used.
