...
url: https://arxiv.org/abs/1802.02656
source: arxiv
doi: null
arxiv_id: '1802.02656'
categories:
...
Table 1: Oracle performance in word error rates that assumes that the true accent ID is known in advance. Word error rates is calculated after decoding with a WFST-graph incorporating a LM; the relative improvement (_rel._) for each model over ASpec are reported in the parenthesis.
...
Table 2: WERs of hard-switch using distorted AID. The _rel._ shows the relative improvement over ASpec; _ind. AID_ applies an independent neural AID trained separately. Our _Proposed Model_ applies the AID jointly learn with multi-accent AMs.
