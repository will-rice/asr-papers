...
1.  1.
    We redesign the downsampling schema and sub-sampling block to increase downsampling to 8x (see Fig.1)
2.  2.
    We optionally replace original self-attention layers with a combination of local attention and global context token post-training, similarly to LongFormer \[5\].
...
1.  1.
    8x downsampling at the start of the encoder, thereby reducing the compute cost of subsequent attention layers by 4x
2.  2.
    Replacement of the original convolution sub-sampling layers with depthwise separable convolutions \[15\]
3.  3.
    Reduction of the number of convolutional filters in the downsampling block to 256
4.  4.
    Reduction of the convolutional kernel size to 9.
