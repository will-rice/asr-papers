...
Robustness or adaptation to new, unconstrained domains is a key challenge for automatic speech recognition (ASR) systems. In multimodal video (*e.g*., TV, online edited videos), the visual stream can provide strong cues for improving the robustness of ASR systems, particularly in cases where the audio is noisy – this is called audiovisual ASR (AV-ASR). Unlike works that simply focus on lip motion \[29, 41, 7, 1, 33, 24, 23, 37\], we investigate the contribution of entire visual frames. This is particularly useful for videos ‘in the wild’, where the mouth is not necessarily visible (*e.g*., egocentric viewpoints, face coverings, and low resolution etc.) \[11\]. The task is illustrated in Figure 1.
...
Loss Function. As the RNN-T decoder in the pretrained ASR model is kept frozen in AVFormer, we adopt the same loss function that is used for ASR pretraining. With an RNN-T decoder, the probability of a transcript $`W = {\{ w_{1},w_{2},\cdots,w_{K}\}}`$ is obtained by marginalizing the probabilities of all valid generation paths $`y`$ (*e.g*., the path with bold arrows in Figure 2), *i.e*.,
