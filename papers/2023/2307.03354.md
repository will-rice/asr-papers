...
arxiv_id: "2307.03354"
title:
  Token-Level Serialized Output Training for Joint Streaming ASR and ST Leveraging
  Textual Alignments
authors:
  - Sara Papi
  - Peidong Wang
...
1.  1.  If a transcription word $`r_{a\hspace{0pt}s\hspace{0pt}r_{i}}`$ and a translation word $`r_{s\hspace{0pt}t_{j}}`$ are uniquely aligned (as the words “Ich” and “I” in Figure 2), they are interleaved following INTER 0.5:
    - $`\Rightarrow`$
      $`\mathbf{r}_{\text{t-SOT}}`$ += \#ASR#, $`r_{a\hspace{0pt}s\hspace{0pt}r_{i}}`$, \#ST#, $`r_{s\hspace{0pt}t_{j}}`$

2.  2.  If $`k`$ consecutive transcription words $`r_{a\hspace{0pt}s\hspace{0pt}r_{i}}`$, $`r_{a\hspace{0pt}s\hspace{0pt}r_{i + 1}}`$,…, $`r_{a\hspace{0pt}s\hspace{0pt}r_{{i + k} - 1}}`$ are aligned with the same translation word $`r_{s\hspace{0pt}t_{j}}`$, we interleave them together as a single word (valid also in the opposite case):
    - $`\Rightarrow`$
...
5.  5.  If no $`\mathbf{r}_{\mathbf{a}\mathbf{s}\mathbf{r}}`$ or $`\mathbf{r}_{\mathbf{s}\mathbf{t}}`$ words are left, we concatenate together all the remaining words of, respectively, $`\mathbf{r}_{\mathbf{s}\mathbf{t}}`$ or $`\mathbf{r}_{\mathbf{a}\mathbf{s}\mathbf{r}}`$:
    - $`\Rightarrow`$
