...
1.  1.
    $`E_{C}`$, the \saycausal encoder, which consumes streamed audio features with no right-context.
2.  2.
    $`E_{N\hspace{0pt}C}`$, the \saynon-causal encoder, which consumes the outputs of $`E_{C}`$ with 900ms of right-context.
3.  3.
    $`D_{C}`$, a decoder for the causal encoder. During inference, this decoder may be used to generate immediate predictions as the user speaks.
4.  4.
    $`D_{N\hspace{0pt}C}`$, a decoder for the non-causal encoder. During inference, this decoder may be used to revise the predictions of the causal decoder with short latency.
