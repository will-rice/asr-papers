...
1.  1.
    F1: BEST-RQ based self-supervised training on YT-U (S0);
2.  2.
    F2: Adding a RNN-T decoder with 6 LSTM layers (S3) and conducting JUST Hydra training with filtered YT-T (D1) and VS-U data (G1);
3.  3.
    F3: Inserting residual adapters between each conformer layers (E3) and finetuning encoder adapter and decoder on target domain supervised data VS-L (E4).
