...
1.  1.
    Train $`f\left( x;{m \odot \theta} \right)`$ for $`T`$ steps, resulting in $`f\left( x;{m \odot \theta_{T}} \right)`$.
2.  2.
    Prune $`p\%`$ of total weights from $`m \odot \theta_{T}`$ that has the smallest magnitudes. Setting the pruned positions in $`m`$ to 0.
3.  3.
    Assign $`\theta_{T}`$ to $`\theta`$ for the next iteration
