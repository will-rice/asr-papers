...
1.  1.
    Vanilla ASR (Baseline): The Whisper_(Tiny) fine-tuned solely on the source-domain data without any channel compensation.
2.  2.
    UNA-GAN: The Whisper_(Tiny) fine-tuned using a dataset generated by UNA-GAN \[[21](https://arxiv.org/html/2409.12386v2#bib.bib21)\], representing a recent baseline approach.
3.  3.
    CADA-GAN: The Whisper_(Tiny) fine-tuned with the augmented dataset generated by CADA-GAN, incorporating both source and simulated target-domain data.
4.  4.
    Topline ASR: The Whisper_(Tiny) fine-tuned directly on the target-domain data, simulating an ideal scenario with abundant labeled target-domain data.
