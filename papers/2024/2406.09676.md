...
1.  1.  a label encoder, $`P_{L}{(\left. Q \middle| W \right.)}`$;
2.  2.  an acoustic encoder, $`P_{A}{(\left. Q \middle| X \right.)}`$;
3.  3.  a label decoder, $`P_{D}{(\left. W \middle| Q \right.)}`$;
4.  4.  a vector quantizer, $`VQ`$.
