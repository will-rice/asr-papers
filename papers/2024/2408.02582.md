...
1.  1.  Randomly select data point $`x \in X`$ as the first centroid $`c_{1}`$.
2.  2.  Each new centroid $`c_{i}`$ is sampled based on the distance \
        probability:$`\frac{D(x)^{2}}{\left. \sum{}_{x \in X} \right.{D(x)^{2}}}`$ where $`D(x)`$ is the distance \
        to the closest centroid from data $`x`$.
3.  3.  Repeat 2 until initializing all of centroids.
