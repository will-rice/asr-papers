...
1.  1.  Set impostor frequencies $`f_{m} = 0`$, $`1 \leq m \leq M`$
2.  2.  For each target i-vector $`{\mathbf{ω}}_{t}`$, $`1 \leq t \leq T`$
...
3.  3.  Sort impostors descendingly based on their $`f_{m}`$
4.  4.  Select the first $`\kappa`$ impostors as the final ones.
