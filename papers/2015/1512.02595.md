...
We find that *sequence-wise* normalization \[37\] overcomes these issues. The recurrent computation is given by
...
where $`\sigma\hspace{0pt}{( \cdot )}`$ is the sigmoid function, $`z`$ and $`r`$ represent the *update* and *reset* gates respectively, and we drop the layer superscripts for simplicity. We differ slightly from the standard GRU in that we multiply the hidden state $`h_{t - 1}`$ by $`U_{h}`$ prior to scaling by the reset gate. This allows for all operations on $`h_{t - 1}`$ to be computed in a single matrix multiplication. The output nonlinearity $`f\hspace{0pt}{( \cdot )}`$ is typically the hyperbolic tangent function $`t\hspace{0pt}a\hspace{0pt}n\hspace{0pt}h`$. However, we find similar performance for $`t\hspace{0pt}a\hspace{0pt}n\hspace{0pt}h`$ and clipped-ReLU nonlinearities and choose to use the clipped-ReLU for simplicity and uniformity with the rest of the network.
...
Non-overlapping bigrams shorten the length of the output transcription and thus allow for a decrease in the length of the unrolled RNN. The sentence *the cat sat* with non-overlapping bigrams is segmented as $`\lbrack{t\hspace{0pt}h},e,{s\hspace{0pt}p\hspace{0pt}a\hspace{0pt}c\hspace{0pt}e},{c\hspace{0pt}a},t,{s\hspace{0pt}p\hspace{0pt}a\hspace{0pt}c\hspace{0pt}e},{s\hspace{0pt}a},t\rbrack`$. Notice that for words with odd number of characters, the last character becomes an unigram and $`s\hspace{0pt}p\hspace{0pt}a\hspace{0pt}c\hspace{0pt}e`$ is treated as an unigram as well. This isomorphism ensures that the same words are always composed of the same bigram and unigram tokens. The output set of bigrams consists of all bigrams that occur in the training set.
