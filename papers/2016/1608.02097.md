---
arxiv_id: '1608.02097'
title: Encoder-decoder with Focus-mechanism for Sequence Labelling Based Spoken Language
  Understanding
authors:
- Su Zhu
- Kai Yu
...
By considering the alignment of the sequence labelling task, the *BLSTM-LSTM with focus* increased the F1-score from 92.73% to 95.79% and achieved an 0.36% improvement (significant level 10%) in comparison to *BLSTM*. We think the *BLSTM-LSTM with focus* has two advantages over the *BLSTM*: 1) the initialization of hidden state of decoder LSTM with $`s_{0} = \overset{\leftarrow}{h_{1}}`$ provides sentence leveraging features; 2) it enables label dependency within the decoder.
