---
arxiv_id: '1603.05800'
title: A Comparison between Deep Neural Nets and Kernel Acoustic Models for Speech
  Recognition
authors:
- Zhiyun Lu
- Dong Guo
...
\[9\] leverage a classical result in harmonic analysis and provide a fast way to approximate $`k\hspace{0pt}( \cdot , \cdot )`$ with *finite*-dimensional features:
...
The *random feature vector* $`\hat{\mathbf{\phi}}`$ is thus composed of scaled cosines of random projections
...
Fig. 2 and 3 suggest that we should select the best acoustic model using both perplexity and entropy. Fig. 4 shows that it is possible to predict WER from *entropy-regularized perplexity*, defined as $`{\log\left( \text{perplexity} \right)} + \text{entropy}`$, namely,
