...
categories:
- cs.CL
arxiv_url: https://arxiv.org/abs/1609.06773
github_repo: ''
source: arxiv-html
converter: pandoc
llm_remediated: false
citations_resolved: 0/0
citations_resolved_at: '2026-07-07T20:26:34+00:00'
references_parsed: 0
arxiv_version: ''
---
...
Index Terms—  end-to-end, speech recognition, connectionist temporal classification, attention, multi-task learning
...
Recent work on end-to-end speech recognition can be categorized into two main approaches: Connectionist Temporal Classification (CTC) \[12, 1, 2, 3\] and attention-based encoder-decoder \[13, 4, 5, 6\]. Both methods address the problem of variable-length input and output sequences. The key idea of CTC is to use intermediate label representation allowing repetitions of labels and occurrences of blank labels to identify no output label. The CTC loss can be efficiently calculated by the forward-backward algorithm, but it still predicts targets for every frame, and assumes that the targets are conditionally independent of each other.

Another approach, the attention-based encoder-decoder directly learns a mapping from acoustic frame to character sequences. At each output time step, the model emits a character conditioned on the inputs and the history of the target character. Since the attention model does not use any conditional independence assumption, it has often shown to improve Character Error Rate (CER) than CTC when no external language model is used \[7\]. However, in real-environment speech recognition tasks, the model shows poor results because the alignment estimated in the attention mechanism is easily corrupted due to the noise. Another issue is that the model is hard to learn from scratch due to the misalignment on longer input sequences, and therefore a windowing technique is commonly used to limit the area explored by the attention mechanism \[7\], but several parameters for windowing need to be determined manually depending on the training data.
...
In this section, we review the CTC in Section 2.1 ‣ 2 JOINT CTC-ATTENTION MECHANISM ‣ JOINT CTC-ATTENTION BASED END-TO-END SPEECH RECOGNITION USING MULTI-TASK LEARNING") and the attention-based encoder-decoder in Section 2.2, addressing the variable ($`T`$) length input frames, $`{\mathbf{x}} = \left( x_{1},\cdots,x_{T} \right)`$, and $`U`$ length output characters, $`{\mathbf{y}} = \left( y_{1},\cdots,y_{U} \right)`$, where $`y_{u} \in \left\{ 1,\cdots,K \right\}`$. $`K`$ is the number of distinct labels. Then, our joint CTC-attention based end-to-end framework will be described in Section 2.3 ‣ 2 JOINT CTC-ATTENTION MECHANISM ‣ JOINT CTC-ATTENTION BASED END-TO-END SPEECH RECOGNITION USING MULTI-TASK LEARNING").

### 2.1 Connectionist temporal classification (CTC)

The key idea of CTC \[12\] is to use intermediate label representation $`\pi = \left( \pi_{1},\cdots,\pi_{T} \right)`$, allowing repetitions of labels and occurrences of a blank label ($`-`$), which represents the special emission without labels, i.e., $`\pi_{t} \in {\left\{ 1,\cdots,K \right\} \cup \left\{ - \right\}}`$. CTC trains the model to maximize $`P\hspace{0pt}\left( {\mathbf{y}} \middle| {\mathbf{x}} \right)`$, the probability distribution over all possible label sequences $`\Phi\hspace{0pt}\left( {\mathbf{y}}' \right)`$:

$`{P\hspace{0pt}\left( {\mathbf{y}} \middle| {\mathbf{x}} \right)} =`$$`{\sum\limits_{{\mathbf{π}} \in {\Phi\hspace{0pt}{({\mathbf{y}}')}}}{P\hspace{0pt}\left( {\mathbf{π}} \middle| {\mathbf{x}} \right)}},`$

where $`{\mathbf{y}}'`$ is a modified label sequence of $`\mathbf{y}`$, which is made by inserting the blank symbols between each label and the beginning and the end for allowing blanks in the output (i.e., $`{{\mathbf{y}} = (c,a,t)},{{\mathbf{y}}' = ( - ,c, - ,a, - ,t, - )}`$).

CTC is generally applied on top of Recurrent Neural Networks (RNNs). Each RNN output unit is interpreted as the probability of observing the corresponding label at particular time. The probability of label sequence $`P\hspace{0pt}\left( {\mathbf{π}} \middle| {\mathbf{x}} \right)`$ is modeled as being conditionally independent by the product of the network outputs:

$`{P\hspace{0pt}\left( {\mathbf{π}} \middle| {\mathbf{x}} \right)} \approx`$$`{\prod\limits_{t = 1}^{T}{P\hspace{0pt}\left( \pi_{t} \middle| {\mathbf{x}} \right)}} = {\prod\limits_{t = 1}^{T}{q_{t}\hspace{0pt}\left( \pi_{t} \right)}}`$

where $`q_{t}\hspace{0pt}\left( \pi_{t} \right)`$ denotes the softmax activation of $`\pi_{t}`$ label in RNN output layer $`q`$ at time $`t`$.

The CTC loss to be minimized is defined as the negative log likelihood of the ground truth character sequence $`{\mathbf{y}}^{\ast}`$, i.e.

$`\mathcal{L}_{\text{CTC}} \triangleq`$$`{- {{\ln P}\hspace{0pt}\left( {\mathbf{y}}^{\ast} \middle| {\mathbf{x}} \right)}}.`$

The probability distribution $`P\hspace{0pt}\left( {\mathbf{y}} \middle| {\mathbf{x}} \right)`$ can be computed efficiently using the forward-backward algorithm as

$`{{P\hspace{0pt}\left( {\mathbf{y}} \middle| {\mathbf{x}} \right)} = {\sum\limits_{u = 1}^{|{\mathbf{y}}'|}\frac{\alpha_{t}\hspace{0pt}(u)\hspace{0pt}\beta_{t}\hspace{0pt}(u)}{q_{t}\hspace{0pt}\left( y_{u}' \right)}}},`$

where $`\alpha_{t}\hspace{0pt}(u)`$ is the forward variable, representing the total probability of all possible prefixes ($`y_{1:u}'`$) that end with the $`u`$-th label, and $`\beta_{t}\hspace{0pt}(u)`$ is the backward variable of all possible suffixes ($`y_{u:U}'`$) that start with the $`u`$-th label. The network can then be trained with standard backpropagation by taking the derivative of the loss function with respect to $`q_{t}\hspace{0pt}(k)`$ for any $`k`$ label including the blank.

Since CTC does not explicitly model inter-label dependencies based on the conditional independence assumption in Eq.(2 ‣ 2 JOINT CTC-ATTENTION MECHANISM ‣ JOINT CTC-ATTENTION BASED END-TO-END SPEECH RECOGNITION USING MULTI-TASK LEARNING")), there are limits to model character-level language information. Therefore, lexicon or language models are commonly incorporated, like the hybrid framework \[2, 3\].
...
$`{P\hspace{0pt}\left( {\mathbf{y}} \middle| {\mathbf{x}} \right)} =`$$`\prod\limits_{u}{P\hspace{0pt}\left( y_{u} \middle| {{\mathbf{x}},y_{1:{u - 1}}} \right)}`$$`{\mathbf{h}} =`$$`\text{Encoder}\hspace{0pt}({\mathbf{x}})`$$`y_{u} \sim`$$`{\text{AttentionDecoder}\hspace{0pt}\left( {\mathbf{h}},y_{1:{u - 1}} \right)}.`$

The framework consists of two RNNs: Encoder and AttentionDecoder, so that it is able to learn two different lengths of sequences based on the cross-entropy criterion. Encoder transforms $`\mathbf{x}`$, to high-level representation $`{\mathbf{h}} = \left( h_{1},\cdots,h_{L} \right)`$ in Eq. (6), then AttentionDecoder produces the probability distribution over characters, $`y_{u}`$, conditioned on $`\mathbf{h}`$ and all the characters seen previously $`y_{1:{u - 1}}`$ in Eq. (7). $`L`$ is the number of the frames of Encoder output, and $`L < T`$. Here, a special start-of-sentence(sos)/end-of-sentence(eos) token is added to the target set, so that the decoder completes the generation of the hypothesis when (eos) is emitted. The loss function of the attention model is computed from Eq. (5) as:

$`\mathcal{L}_{\text{Attention}} \triangleq {- {{\ln P}\hspace{0pt}\left( {\mathbf{y}}^{\ast} \middle| {\mathbf{x}} \right)}} = {- {\sum\limits_{u}{{\ln P}\hspace{0pt}\left( y_{u}^{\ast} \middle| {{\mathbf{x}},y_{1:{u - 1}}^{\ast}} \right)}}}`$

where $`y_{1:{u - 1}}^{\ast}`$ is the ground truth of the previous characters.

The attention mechanism aids in the decoding procedure by integrating all the inputs $`\mathbf{h}`$ into $`c_{u}`$ based on their attention weight vectors $`a_{u} \in {\mathbb{R}}_{+}^{L}`$ over input $`L`$ identifying where to focus at output step $`u`$. The following equations represent how to compute $`a_{u}`$ and $`c_{u}`$:

$`e_{u,l} =`$$`\left\{ \begin{array}{lc}
\text{content-based:} & \\
{w^{T}\hspace{0pt}{\tanh\left( {{W\hspace{0pt}s_{u - 1}} + {V\hspace{0pt}h_{l}} + b} \right)}} & \\
\text{location-based:} & \\
{f_{u} = {F \ast a_{u - 1}}} & \\
{w^{T}\hspace{0pt}{\tanh\left( {{W\hspace{0pt}s_{u - 1}} + {V\hspace{0pt}h_{l}} + {U\hspace{0pt}f_{u,l}} + b} \right)}} & 
\end{array} \right.`$$`a_{u,l} =`$$`\frac{\exp\left( {\gamma\hspace{0pt}e_{u,l}} \right)}{\left. \sum{}_{l}{\exp\left( {\gamma\hspace{0pt}e_{u,l}} \right)} \right.}`$$`c_{u} =`$$`\sum\limits_{l}{a_{u,l}\hspace{0pt}h_{l}}`$
...
$`a_{u}`$ can be computed by the softmax of energy $`e_{u,l}`$ from two types of attention mechanisms: content-based and location-based in Eq. (9). Both depend on the decoder state, $`s_{u - 1}`$, and the content of input, $`h_{l}`$. The location-based attention mechanism additionally uses convolutional feature vectors $`f_{u,l}`$ extracted from the previous attention $`a_{u - 1}`$ by convolving with matrix $`F`$ along the time axis \[5\].

With $`c_{u}`$, $`s_{u - 1}`$, and $`y_{u - 1}`$, the decoder generates next label $`y_{u}`$ and updates the state as:

$`y_{u} \sim`$$`\text{Generate}\hspace{0pt}\left( c_{u},s_{u - 1} \right)`$$`s_{u} =`$$`{\text{Recurrency}\hspace{0pt}\left( s_{u - 1},c_{u},y_{u} \right)},`$
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/char_acc_2.png)
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/attention_1_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(a) Attention 1 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/attention_3_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(b) Attention 3 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/attention_5_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(c) Attention 5 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/attention_7_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(d) Attention 7 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/attention_9_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(e) Attention 9 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/mtl_1_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(f) MTL 1 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/mtl_3_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(g) MTL 3 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/mtl_5_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(h) MTL 5 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/mtl_7_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)

(i) MTL 7 epoch

![Refer to caption](https://ar5iv.labs.arxiv.org/html/1609.06773/assets/figure/mtl_9_F05_442C020U_CAF_REAL_convf10_char_Montreal.png)
...
The encoder was a 4-layer Bidirectional Long Short-Term Memory (BLSTM) \[17, 18\] with 320 cells in each layer and direction, and linear projection layer is followed by each BLSTM layer. The top two layers of the encoder read every second hidden state in the network below, reducing the utterance length by the factor of 4, $`L = \left. T/4 \right.`$. The decoder was 1-layer LSTM with 320 cells. In case of the location-based attention model, 10 centered convolution filters of width 100 were used to extract the convolutional features. We used the sharpening factor $`\gamma = 2`$. The AdaDelta algorithm \[19\] with gradient clipping \[20\] was used for optimization. All the weights are initialized with the range \[-0.1, 0.1\] of uniform distribution. For our MTL, we tested three different task weights, $`\lambda`$: 0.2, 0.5, and 0.8.

For decoding of the attention and MTL models, we used a beam search algorithm similar to \[21\] with the beam size 20 to reduce the computation cost. We adjusted the score by adding a length penalty, $`\text{length(hyp)} \ast 0.3`$ for CHiME-4 and $`\text{length(hyp)} \ast 0.1`$ for WSJ experiments. For decoding of CTC model, we took the sequence of most likely outputs. Note that we do not use any lexicon or language models. Our framework is implemented with the Chainer library \[22, 23\].
...
- \[7\] Dzmitry Bahdanau, Jan Chorowski, Dmitriy Serdyuk, Philemon Brakel, and Yoshua Bengio, “End-to-end attention-based large vocabulary speech recognition,” arXiv preprint arXiv:1508.04395, 2015.
