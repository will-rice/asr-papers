...
1.  1.
    OpenMIC \[27\]: OpenMIC is a multi-label instrument classification dataset containing 15k samples total with provided train/valid/test splits as well as masks for strong positive and negative examples for each class. We follow similar setup as the official baseline³³3https://github.com/cosmir/openmic-2018 by training 20 binary classifiers.
2.  2.
    Extended Ballroom \[28\]: Extended Ballroom (4k samples) is a multi-class dance genre classification dataset. We follow the same setup as \[30\] by removing 4 categories due to dataset imbalance, resulting in only using 9 categories.
3.  3.
    FMA Small \[29\]: FMA Small (8k samples) is a multi-class music genre classification dataset with 8 genre categories.
