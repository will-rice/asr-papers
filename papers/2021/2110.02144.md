...
where “$`\ast`$” denotes convolution operator. Dereverberation is thus defined as a *blind deconvolution*, that is, the task of recovering $`x\hspace{0pt}(t)`$ using observations of $`y\hspace{0pt}(t)`$ in eq. (1) when the $`h\hspace{0pt}(t)`$ is unknown. Notice that by splitting the room impulse response $`h\hspace{0pt}(t)`$ in *early reflections* $`h_{\text{early}}\hspace{0pt}(t)`$ and *late reflections* $`h_{\text{late}}\hspace{0pt}(t)`$, eq. (1) can be expressed as
