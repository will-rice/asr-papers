...
1.  1.  Mean pooling: $`o = {\sum_{t = 1}^{T}\left. c_{t}/T \right.}`$.
2.  2.  Standard deviation pooling: $`o = \sqrt{\sum_{t = 1}^{T}\left. \left( {c_{t} - \mu} \right)^{2}/T \right.}`$
3.  3.  Self-attention pooling \[31\]:
...
4.  4.  CLS-token pooling \[32\]: Adding a special \[CLS\] token in the beginning of sequence and set $`o = c_{1}`$.
...
1.  1.  wav2vec 2.0 En is trained on English only.
2.  2.  XLSR is trained with all unlabeled audios.
