---
arxiv_id: '2106.00856'
title: A Neural Acoustic Echo Canceller Optimized Using An Automatic Speech Recognizer
  And Large Scale Synthetic Data
authors:
- Nathan Howard
- Alex Park
...
- eess.AS
- cs.SD
arxiv_url: https://arxiv.org/abs/2106.00856
github_repo: ''
source: arxiv-html
converter: pandoc
llm_remediated: false
citations_resolved: 0/0
citations_resolved_at: '2026-07-07T19:43:25+00:00'
references_parsed: 0
arxiv_version: ''
---
...
The speech encoder is similar to the encoder described in \[8\], which takes a sequence of speech features as input and produces a high dimensional hidden representation sequence. We compute feature frames for each of the probe and reference signals, then stack each frame depthwise to create an input tensor that has shape $`\lbrack B,T,80,2\rbrack`$, where $`B`$ is the batch size and $`T`$ is the number of frames. For the encoder used in this work, we used 3 unidirectional LSTM layers, each with 512 hidden dimensions, and no temporal downsampling, so the number of hidden representation has the same number of frames as the input.
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/2106.00856/assets/x1.png)
...
Our initial experiments used purely spectral loss when training the network. This loss is computed by summing the mean $`L\hspace{0pt}1`$ and mean $`L\hspace{0pt}2`$ (or MSE) distance between the target spectral features and the output of the decoder, both before and after applying the post-net correction.
...
``` math
{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}} = {{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{s\hspace{0pt}p\hspace{0pt}e\hspace{0pt}c\hspace{0pt}t\hspace{0pt}r\hspace{0pt}a\hspace{0pt}l} + {\lambda\hspace{0pt}{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{A\hspace{0pt}S\hspace{0pt}R}}}
```
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/2106.00856/assets/x2.png)
...
One concern with optimizing the AEC model using a pretrained ASR encoder is that the AEC will overfit to the idiosyncrasies of that specific ASR encoder and produce outputs that are mismatched when presented to other ASR models. To measure this effect, we trained two AEC models, one with and one without $`{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{A\hspace{0pt}S\hspace{0pt}R}`$, and evaluated the outputs on three different ASR models, of which only one was used for calculating $`{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{A\hspace{0pt}S\hspace{0pt}R}`$ during training.
...
Figure 3 shows the WER of each of the ASR models on the outputs of the AEC models. Though only the in-domain CNN speech encoder was used to calculate $`{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{A\hspace{0pt}S\hspace{0pt}R}`$ for the AEC model incorporating that loss, we observe consistent improvements for the other two ASR models across all SNRs as well. This improvement holds despite significant differences in model structure, training data, and frontend configuration. As expected, we observe the largest improvements for the matched ASR encoder (CNN), followed by the in-domain LSTM recognizer, and smaller, but still significant gains for the out-of-domain model.

![Refer to caption](https://ar5iv.labs.arxiv.org/html/2106.00856/assets/x3.png)

Fig. 3: ASR Loss Robustness. Word Error rates for different ASR models on AEC model outputs, comparing outcomes when training with and without $`{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{A\hspace{0pt}S\hspace{0pt}R}`$. Only the LibriSpeech CNN model (blue line) was used for computing $`{\mathtt{l}\mathtt{o}\mathtt{s}\mathtt{s}}_{A\hspace{0pt}S\hspace{0pt}R}`$ during training.
...
- \[22\] Y. Avargel and I. Cohen, “Performance analysis of cross-band adaptation for subband acoustic echo cancellation,” in Inter- national Workshop on Acoustic Echo and Noise Control, 09 2006.
