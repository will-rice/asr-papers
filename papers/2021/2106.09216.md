...
1.  1.
    For each layer of the current model, we induce a new sub-model by removing the layer from the model. For example, from a given model $`\left\{ 2,3,4 \right\}`$, we induce three sub-models: $`\left\{ 2,3 \right\},\left\{ 2,4 \right\},\left\{ 3,4 \right\}`$.
2.  2.
    We also induce an intermediate sub-model $`\left\{ 1,\ldots,{k - 1} \right\}`$ from the original model. In the current example, the intermediate sub-model is $`\left\{ 1,2 \right\}`$.
3.  3.
