...
1.  1.
    We demonstrate that fine-tuning wav2vec 2.0 with a CTC loss can improve the state-of-the-art for automatic recognition of suprasegmentals, including syllables, tones, and pitch accents.
2.  2.
    Utilizing segmental information, by employing tonal finals or tonal syllables as recognition units, can significantly improve Mandarin tone recognition. Language models are helpful when tonal syllables are used as recognition units, but not helpful when tones are recognition units.
3.  3.
    Mandarin tone recognition benefits from English phoneme recognition by combing the two tasks in fine-tuning wav2vec 2.0.
