...
FFNN denotes the learnable feed-forward neural network, $`s_{i}^{l}`$ is the predicted PLL approximation, and PLL$`\left( E_{i} \right)`$ is precomputed offline using Eq. (1 ‣ 2.1 BERT Without Discriminative Training ‣ 2 Approach ‣ RescoreBERT: Discriminative Speech Recognition Rescoring with BERT")). Note that the PLL can be computed by a larger *teacher* model.
...
In Section 4, we show results using this approach with $`\mathcal{L}_{\text{MWER}}`$, labeled “*MWER Only*”, where we perform MWER training on a pretrained BERT with domain adaptation.
...
In Section 4, we show the results using this approach with $`\mathcal{L}_{\text{MWER}}`$ and $`\mathcal{L}_{\text{MWED}}`$, which are named *MD-MWER* and *MD-MWED*, respectively.
...
Tables 1(a) and (b) show the evaluation results on both LibriSpeech and internal datasets. Here, *PLL* denotes the approach in Section 2.1.1 ‣ 2.1 BERT Without Discriminative Training ‣ 2 Approach ‣ RescoreBERT: Discriminative Speech Recognition Rescoring with BERT") which is computationally expensive; *MD* denotes the approach in Section 2.1.2 ‣ 2.1 BERT Without Discriminative Training ‣ 2 Approach ‣ RescoreBERT: Discriminative Speech Recognition Rescoring with BERT") that distills the PLL score; *MWER Only* denotes the approach described in Section 2.2.2 that trains BERT with the MWER objective only; *MD-MWER* and *MD-MWED* denote the approaches described in Section 2.2.3 that incorporate MLM into discriminative training with MWER and MWED loss functions (as in Section 2.2.1), respectively.
...
(a) WER on the two test sets of LibriSpeech using BERT_(Base). Numbers inside parentheses are relative improvements compared to the baseline.
