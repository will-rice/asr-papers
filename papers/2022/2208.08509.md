...
1.  1.
    From (E1), we conclude that HuBERT is (around $`25\%`$) slower than wav2vec2 but on simulated noise, it’s more robust than wav2vec2. Similarly, DistilHuBERT is more robust than wav2vec2.
2.  2.
    In the additive noise version of (E2A) we observe that layers 6-8 seem very noise sensitive while other layers seem relatively noise-robust. This suggests that layers 6-8 learn higher level information such as semantics, context or meaning. This matches the observations of \[3\].
3.  3.
    Almost all layers of wav2vec2 are surprisingly robust on multiplicative noise injection of (E2B), except for layer $`11`$ which behaves in a unusual manner.
4.  4.
    The experiments (E2B) suggests that the wav2vec2 model “eats” up the noise as we go up the layers. This is true for all layers except layer 11, where intriguing things seem to happen.
...
1.  1.
    Background noise: Background noise is an interesting direction for further study, which perhaps maybe a more realistic kind of noise. To generate realistic background noise, some works, e.g. \[9\] mixed other speech datasets with the main one, using various techniques. So it would be interesting to repeat our experiments in this setting.
2.  2.
...
- \[6\] J. S. Garofolo, L. F. Lamel, W. M. Fisher, J. G. Fiscus, and D. S. Pallett, “Darpa timit acoustic-phonetic continous speech corpus cd-rom. nist speech disc 1-1.1,” *NASA STI/Recon technical report n*, vol. 93, p. 27403, 1993.
