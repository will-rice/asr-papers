...
arxiv_id: "2305.18824"
title:
  Graph Neural Networks for Contextual ASR with the Tree-Constrained Pointer
  Generator
authors:
  - Guangzhi Sun
  - Chao Zhang
...
TABLE III: WER (R\_(s)-WER) on AMI test set using the audio-visual contextual ASR pipeline with 10% of AMI training set. Baseline is the standard AED and N-T systems, and AMI baseline (the first row) specifically refers to the standard system trained from scratch on the full AMI training set.
