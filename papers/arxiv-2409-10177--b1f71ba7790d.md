...
1.  1.  The audio is fed into a feature extraction model that is pre-trained with CTC to generate frame-wise label probability over the whole alphabet.
2.  2.  From the probability, a trellis matrix is generated to represent the probability of labels occurring at each time frame. The trellis at point $`(t,j)`$ (where $`0 \leq t \leq {T - {1and\;0}} \leq j \leq {U - 1}`$) represents the maximal probability that the first $`j - 1`$ labels of the transcripts are aligned to the first $`t - 1`$ timeframes of the audio. The trellis is calculated in the log domain to avoid numerical instability.
...
3.  3.  In the third step, the path with the highest probability in the trellis is traced back. This path begins at position $`{(0,0)}andendsatposition{(T,U)}`$ to encompass the entire audio and transcript.
