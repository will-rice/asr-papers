...
1.  1.  Method-level significance: To assess the overall influence of the different methodological interventions (such as fine-tuning, LM integration, or ablated parameters) on model performance. In this case, we will compare the dataset-level WER scores across methods. This analysis will help us assess the impact of the method employed.
2.  2.  Sentence-level significance: To analyze the impact of individual result values across specific datasets and model sizes, ensuring a fine-grained evaluation of performance variations. In this method, we will compare individual sentence-level WERs across the different methods. These comparisons will be displayed in the results tables as a superscript in the scores.
