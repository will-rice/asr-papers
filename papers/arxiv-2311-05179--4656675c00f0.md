...
arxiv_id: "2311.05179"
title:
  Improving Whispered Speech Recognition Performance using Pseudo-whispered based
  Data Augmentation
authors:
  - Zhaofeng Lin
  - Tanvina Patel
...
1.  1.  Removing the glottal source using GFM-IAIF-based Glottal Inverse Filtering (see section 2.3.1),
2.  2.  Increasing the formant bandwidth and up-shifting the formant frequencies using moving average filtering (see section 2.3.2).
...
1.  1.  Training data: TIMIT plus the normal speech from wTIMIT (TM+wTM-n) vs. TIMIT plus both normal and whispered speech from wTIMIT (TM+wTM-wn);
2.  2.  Data augmentation: none vs. speed perturbation (SP) \[28\] at 90% and 110% of the original rate of the training data and SpecAugment \[29\] which was used with a maximum width of each time and frequency mask of $`T = 20`$, $`F = 10`$, respectively;
3.  3.  Dictionary types and sizes: six models used character-level dictionaries, and the remaining two used a Byte Pair Encoding (BPE) token dictionary \[30\];
4.  4.  Model architectures: we compared the Hybrid-CTC \[31\] and Conformer \[32\] architectures (from the LibriSpeech recipe from the ESPNet framework).
