...
arxiv_id: "2306.13277"
title:
  Meta-Gating Framework for Fast and Continuous Resource Optimization in Dynamic
...
Lipschitz continuity and smoothness²²2This assumption is widely used in the analysis of deep learning, such as \[37, 38\]. _Function $`g{(\theta)}`$ is $`G`$-Lipschitz continuous if $`{\|{{g{(\theta_{1})}} - {g{(\theta*{2})}}}\|}*{2} \leq {G{\|{\theta*{1} - \theta*{2}}\|}_{2}}`$ with a constant $`G`$. And $`g{(\theta)}`$ is called $`L`$-smooth if $`{\|{{\bigtriangledown g{(\theta_{1})}} - {\bigtriangledown g{(\theta*{2})}}}\|}*{2} \leq {L{\|{\theta*{1} - \theta*{2}}\|}_{2}}`$ with a constant $`L`$_.

###### Definition 2

Excess Risk. _ER$`{({\mathbf{θ}}*{c}^{J*{q}})} = {{\mathbb{E}}*{c \sim \mathcal{C}}{\mathbb{E}}*{D*{c}}{\lbrack{{\mathcal{L}{({\mathbf{θ}}*{c}^{J\_{q}},\mathbf{\phi}^{_})}} - {\mathcal{L}{({\mathbf{θ}}_{c}^{*},\mathbf{\phi}^{*})}}}\rbrack}}`$, where $`\mathcal{L}{( \cdot )}`$ denotes the expected loss on $`{\mathbf{θ}}_{c}`$\*.
...
(Testing Performance Analysis). _Suppose that the loss function $`\mathcal{L}`$ is $`G`$-Lipschitz continuous and $`L`$-smooth w.r.t. both the inner and outer network parameters ($`\mathbf{θ}`$ and $`\mathbf{\phi}`$). Assume that $`\beta`$ obeys $`\beta \leq \frac{1}{L}`$ and denote $`\rho = {1 + {2\beta L}}`$. Then for any $`c \sim \mathcal{C}`$ and $`D_{c}`$ with size $`N*{m}^{te}`$, we have*
...
Expected Population Gradient. _Let EPG$`{({\mathbf{θ}}*{c}^{J*{q}})} = {\mathbb{E}}*{c \sim \mathcal{C}}\left\lbrack \parallel {\mathbb{E}}*{D*{c}}{\lbrack\bigtriangledown\mathcal{L}{({\mathbf{θ}}*{c}^{J\_{q}}|\mathbf{\phi}^{_})}\rbrack} \parallel*{2}^{2} \right\rbrack`$ denote the gradient of the loss function $`\mathcal{L}{({\mathbf{θ}}*{c},\mathbf{\phi}^{_})}`$ on all samples $`D\_{c}`$ and all channels $`c \sim \mathcal{C}`$_.
...
(First-order Optimality Analysis). _Suppose that the loss function $`\mathcal{L}`$ is $`G`$-Lipschitz continuous and $`L`$-smooth w.r.t. both the inner and outer network parameters ($`\mathbf{θ}`$ and $`\mathbf{\phi}`$). Assume that $`\beta`$ obeys $`\beta \leq \frac{1}{L}`$ and denote $`\rho = {1 + {2\beta L}}`$. Then for any $`c \sim \mathcal{C}`$ and $`D_{c}`$ with size $`N*{m}^{te}`$, we have*
...
_Assume that function $`\mathcal{L}{( \cdot )}`$ is $`L`$-smooth in $`{\mathbf{θ}}_{c}`$. If $`\beta \leq \frac{1}{L}`$, then it holds for any channel $`c`$ and any parameters $`{\mathbf{θ}}_{c}`$ that_
...
_where $`{\mathbf{θ}}^{_}`$ denotes the learned initializations of the inner network and $`{\mathbf{θ}}_{c}^{J_{q}} = {{\mathbf{θ}}^{_} - {\beta\left( {{\bigtriangledown\mathcal{L}_{D*{c}}{({\mathbf{θ}}^{*},\mathbf{\phi}^{_})}} + {\sum*{t = 1}^{J*{q} - 1}{\bigtriangledown\mathcal{L}*{D*{c}}{({\mathbf{θ}}\_{c}^{t},\mathbf{\phi}^{_})}}}} \right)}}`$\*.

###### Lemma 2

_Assume that function $`\mathcal{L}{( \cdot )}`$ is $`L`$-smooth in $`{\mathbf{θ}}_{c}`$. We denote the expected and empirical losses on $`D*{c}`$ as $`\mathcal{L}{({\mathbf{θ}}*{c})}`$ and $`\mathcal{L}_{D_{c}}{({\mathbf{θ}}_{c})}`$, respectively. Here $`\beta \leq \frac{1}{L}`$, given a channel $`c`$, considering the empirical minimization problem in (26)-(27)._
...
- \[9\] J. Kirkpatrick, _et al_., “Overcoming catastrophic forgetting in neural networks,” in Proc. PNAS, 2017, pp. 3521–3526.
...
- \[21\] Andrei A Rusu, _et al._, “Progressive neural networks,” arXiv preprint arXiv:1606.04671, 2016.
