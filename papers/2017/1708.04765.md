...
1.  1.
    First, spoken text are commonly shorter and less grammatical, not comply with rigid syntactic constraints. Sentence elements like subject or object are often omitted. It is very context-dependent. Also, there are no punctuation marks in the texts. It, therefore, is non–trivial to segment and parse spoken sentences correctly.
2.  2.
    Second, conversational speech contains a lot of self-correcting, hesitation, and stutter. This is one of the main reasons that causes nested FS. $`f\hspace{0pt}s_{9}`$ and $`f\hspace{0pt}s_{13}`$ within turn $`t_{4}`$ in Table II are the instances.
3.  3.
    Third, the output text of Automatic Speech Recognition are all in lowercase and bearing a small percentage of errors.
...
Let $`t = \left\{ t_{1},t_{2},{\ldots\hspace{0pt}t_{n}} \right\}`$ be turns and $`y = \left\{ B,I \right\}`$ be per-token output tags. We predict the most likely y, given a conditional model $`P\hspace{0pt}{(\left. y \middle| t \right.)}`$.
