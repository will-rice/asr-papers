...
*^(†) ^(‡): Contributed equally as the first and second co-authors, respectively* On leave at Google Inc. New York.
...
2.  2.
    “Capped Log Loss:” For any value of $`\lambda \geq 0`$, we can define:
...
3.  3.
    “Top-k Log Loss:” For this loss, assume that the heldout examples $`({\mathbf{x}}_{i},y_{i})`$ are sorted in descending order of their $`p\hspace{0pt}{(\left. y_{i} \middle| {\mathbf{x}}_{i} \right.)}`$ values. Now, for any positive integer $`k \leq N`$, we can define the “Top-k Log Loss” as follows:
...
1.  1.
    Cross-entropy: Given examples, $`\{{{{({\mathbf{x}}_{i},y_{i})},i} = {1\hspace{0pt}\ldots\hspace{0pt}N}}\}`$, the cross-entropy is defined as

    ``` math
    {- {\frac{1}{N}\hspace{0pt}{\sum\limits_{i = 1}^{N}{{\log p}\hspace{0pt}{(\left. y_{i} \middle| {\mathbf{x}}_{i} \right.)}}}}}.
    ```
2.  2.
    Average Entropy: The average entropy of a model is defined as
...
3.  3.
    Entropy Regularized Log Loss (ERLL): Defined in Section 5. We use $`\beta = 1`$ unless specified otherwise.
4.  4.
    Classification Error: The classification error is defined as
    ...
    ```
5.  5.
    Token Error Rate (TER): We feed the predictions of the acoustic models, which correspond to probability distributions over the phonetic states, to the rest of the ASR pipeline and calculate the misalignment between the decoder’s outputs and the ground-truth transcriptions. For Bengali and BN-50, we measure the error in terms of the word error rate (WER), for Cantonese we use the character error rate (CER), and for TIMIT we use the phone error rate (PER). We use the term “token error rate” (TER) to refer, for each dataset, to its corresponding metric.
...
- Xiong et al. \[2016\] W. Xiong, Jasha Droppo, Xuedong Huang, Frank Seide, Mike Seltzer, Andreas Stolcke, Dong Yu, and Geoffrey Zweig. Achieving human parity in conversational speech recognition. *CoRR*, abs/1610.05256, 2016. URL [http://arxiv.org/abs/1610.05256](http://arxiv.org/abs/1610.05256).
...
- Yu et al. \[2015\] Felix X. Yu, Sanjiv Kumar, Henry A. Rowley, and Shih-Fu Chang. Compact nonlinear maps and circulant extensions. *CoRR*, abs/1503.03893, 2015. URL [http://arxiv.org/abs/1503.03893](http://arxiv.org/abs/1503.03893).
