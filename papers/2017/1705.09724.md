...
1.  1.
    $`H`$ maps multiple HMM states to context-dependent triphones.
2.  2.
    $`C`$ maps triphone sequences to monophones.
3.  3.
    $`L`$ is the lexicon, it maps monophone sequences to words.
4.  4.
    $`G`$ represents a language model FST converted from an ARPA format n-gram model.
