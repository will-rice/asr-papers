...
arxiv_id: "1606.06864"
title:
  A Curriculum Learning Method for Improved Noise Robustness in Automatic Speech
  Recognition
authors:
  - Stefan Braun
  - Daniel Neil
...
1.  1.  Mix every training sample with a randomly selected noise segment from a large pink noise pool to create a resulting sample at a randomly chosen SNR level between $`0`$ to $`50`$ dB.
2.  2.  Extract audio features (e.g. filterbank features) for the noise-corrupted audio to obtain the training data for the current epoch.
