...
Figure 1 shows an example of keyword prefix tree with sample keyword set {cat, car, coat}. Each node has its token as a state and a word index. First node represents the root node, $`n_{0}`$, and the colored nodes are leaf*nodes. Dashed node means going back to the $`n*{0}`$ since there is no child node.
