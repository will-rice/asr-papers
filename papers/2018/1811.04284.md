---
arxiv_id: '1811.04284'
title: Improving End-to-end Speech Recognition with Pronunciation-assisted Sub-word
  Modeling
authors:
- Hainan Xu
- Shuoyang Ding
...
1.  1.
    Using an aligner to generate a letter-phoneme alignment from a pronunciation dictionary
2.  2.
    Extract consistent letter-phoneme pairs from alignment
3.  3.
    Collect letter-sequence statistics from the consistent letter-phoneme pairs
...
1.  1.
    min-count constraint: $`L`$ must occur at least $`N`$ times in the training corpus,
2.  2.
    proportion constraint: of all the words containing $`L`$ in the corpus, at least a certain fraction $`p`$ of all occurrences is mapped to a particular phone-sequence $`P`$.
