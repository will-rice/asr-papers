...
1.  1.
    Methods that regularize the activations or alter the structure of the network during training time to align with interpretability metrics \[7, 8, 9, 10\].
2.  2.
    Methods that operate on pre-trained models to identify inputs or activations that strongly contribute to the network’s output \[11, 12, 13, 14, 15, 16, 17\].
3.  3.
    Methods such as DeepDream or image inversion that attempts to understand properties of the filter weights that define model behavior \[18, 19\].
