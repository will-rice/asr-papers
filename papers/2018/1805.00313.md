...
where $`\mathbf{z}_{i\hspace{0pt}k}^{x}`$ denotes the hidden representation, $`\mathbf{W}_{k}^{x}`$ and $`\mathbf{b}_{k}^{x}`$, $`k = {1,\cdots,K}`$, are weight matrices and biases, respectively. The superscripts $`t`$ and $`b`$ refer to *top* and *bottom*. $`s:{{\mathbb{R}}\mapsto{\mathbb{R}}}`$ is a non-linear function applied element wise⁵⁵5In this work, we use the sigmoid function $`{s\hspace{0pt}{(x)}} = {1/{({1 + e^{- x}})}}`$.. We treat the output of the $`K`$-th layer as the latent representations for tops and bottoms, i.e., $`{{\overset{\sim}{\mathbf{z}}}_{i}^{x} = \mathbf{z}_{i\hspace{0pt}K}^{x} \in {\mathbb{R}}^{D_{l}}},{x = {\{ t,b\}}}`$, where $`D_{l}`$ denotes the dimensionality of the latent compatibility space. Accordingly, we can measure the compatibility between top $`t_{i}`$ and bottom $`b_{j}`$ as follows,
