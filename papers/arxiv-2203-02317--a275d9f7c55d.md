...
where $`\mathbf{0}`$ denotes an identity vector. These distributions are trained along with the primary $`P_{\text{rnnt}}`$ as elaborated in Sec 3.4. The distance between $`P_{\text{ILM}},P_{\text{IAM}}`$ can be any divergence metric. We use _KL-divergence_:
