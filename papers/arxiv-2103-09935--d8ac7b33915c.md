...
categories:
  ...
  - cs.SD
  - eess.AS
arxiv_url: https://arxiv.org/abs/2103.09935
github_repo: ""
source: arxiv-html
converter: pandoc
llm_remediated: false
citations_resolved: 0/0
citations_resolved_at: "2026-07-07T19:47:10+00:00"
references_parsed: 0
arxiv_version: ""
---
...
Borrowing some notations from \[1\], RNN-Ts model the conditional distribution $`p\hspace{0pt}\left( \mathbf{y} \middle| \mathbf{x} \right)`$ of an output sequence $`\mathbf{y} = \left( y_{1},\ldots,y_{U} \right) \in \mathcal{Y}^{\ast}`$ of length $`U`$ given an input sequence $`\mathbf{x} = \left( x_{1},\ldots,x_{T} \right) \in \mathcal{X}^{\ast}`$ of length $`T`$. The elements of $`\mathbf{x}`$ are typically continuous multidimensional vectors whereas the elements of $`\mathbf{y}`$ belong to an output space which is typically discrete. $`p\hspace{0pt}\left( \mathbf{y} \middle| \mathbf{x} \right)`$ is expressed as a sum over all possible alignments $`\mathbf{a} = \left( a_{1},\ldots,a_{T + U} \right)`$ that are consistent with $`\mathbf{y}`$:
...
The elements of $`\mathbf{a}`$ belong to the augmented vocabulary $`\overline{\mathcal{Y}} = {\mathcal{Y} \cup \left\{ \phi \right\}}`$ where $`\phi`$ (called BLANK) denotes the null output. The mapping $`\mathcal{B}:{{\overline{\mathcal{Y}}}^{\ast}\rightarrow\mathcal{Y}^{\ast}}`$ is defined by $`{\mathcal{B}\hspace{0pt}(\mathbf{a})} = \mathbf{y}`$. For example, if $`\mathbf{y} = (C,A,T)`$ and $`\mathbf{x} = \left( x_{1},x_{2},x_{3},x_{4} \right)`$ valid alignments include $`(\phi,C,\phi,A,\phi,T,\phi)`$, $`(\phi,\phi,\phi,\phi,C,A,T)`$, $`(C,A,T,\phi,\phi,\phi,\phi)`$, etc. Furthermore, $`p\hspace{0pt}\left( \mathbf{a} \middle| \mathbf{x} \right)`$ can be factorized as follows:

$`{p\hspace{0pt}\left( \mathbf{a} \middle| \mathbf{x} \right)} = {p\hspace{0pt}\left( \mathbf{a} \middle| \mathbf{h} \right)}\overset{\Delta}{=}{\prod\limits_{i = 1}^{T + U}{p\hspace{0pt}\left( a_{i} \middle| {h_{t_{i}},{\mathcal{B}\hspace{0pt}\left( a_{1},\ldots,a_{i - 1} \right)}} \right)}} =`$$`{\prod\limits_{i = 1}^{T + U}{p\hspace{0pt}\left( a_{i} \middle| {h_{t_{i}},y_{0},\ldots,y_{u_{i - 1}}} \right)}} = {\prod\limits_{i = 1}^{T + U}{p\hspace{0pt}\left( a_{i} \middle| {h_{t_{i}},g_{u_{i}}} \right)}}`$\

where: $`\mathbf{h} = \left( h_{1},\ldots,h_{T} \right) = {E\hspace{0pt}n\hspace{0pt}c\hspace{0pt}o\hspace{0pt}d\hspace{0pt}e\hspace{0pt}r\hspace{0pt}(\mathbf{x})}`$ is an embedding of the input sequence computed by an encoder network, $`\mathbf{g} = \left( g_{1},\ldots,g_{U} \right)`$ is an embedding of the output sequence computed by a prediction network via the recursion $`g_{u} = {P\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d\hspace{0pt}i\hspace{0pt}c\hspace{0pt}t\hspace{0pt}i\hspace{0pt}o\hspace{0pt}n\hspace{0pt}\left( g_{u - 1},y_{u - 1} \right)}`$ (with the convention $`{g_{0} = \mathbf{0}},{y_{0} = \phi}`$), and $`p\hspace{0pt}\left( a_{i} \middle| {h_{t_{i}},g_{u_{i}}} \right)`$ is the predictive output distribution over $`\overline{\mathcal{Y}}`$ computed by a joint network which is commonly implemented as:

```math
p\left( \cdot \middle| h_{t},g_{u} \right) = {softmax}\left\lbrack \mathbf{W}^{o\hspace{0pt}u\hspace{0pt}t}\tanh\left( \mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c}h_{t} + \mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}g_{u} + b \right) \right\rbrack
```

\

$`\mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c} \in {I\hspace{0pt}R^{J \times E}}`$, $`\mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d} \in {I\hspace{0pt}R^{J \times P}}`$ are linear projections that map $`h_{t} \in {I\hspace{0pt}R^{E}}`$ and $`g_{u} \in {I\hspace{0pt}R^{P}}`$ to a joint subspace which, after addition and tanh, is mapped to the output space via $`\mathbf{W}^{o\hspace{0pt}u\hspace{0pt}t} \in {I\hspace{0pt}R^{{|\overline{\mathcal{Y}}|} \times J}}`$. RNN-Ts are typically trained to a minimize the negative log-likelihood (NLL) loss $`- {{\log p}\hspace{0pt}\left( \mathbf{y} \middle| \mathbf{x} \right)}`$. From (1), calculating the sum by direct enumeration is intractable because the number of all possible alignments of length $`T + U`$ is $`\left| {\mathcal{B}^{- 1}\hspace{0pt}(\mathbf{y})} \right| = \binom{T + U}{U} = \frac{\left( {T + U} \right)!}{{T!}\hspace{0pt}{U!}} \geq \left( {1 + \frac{T}{U}} \right)^{U}`$ where $`\binom{n}{k}`$ denotes the binomial coefficient $`n`$ choose $`k`$. Luckily, the factorization in (2) allows for an efficient forward-backward algorithm with $`T \times U`$ complexity for both loss and gradient computation \[1, 24\].
...
```math
p\left( \cdot \middle| h_{t},g_{u} \right) = {softmax}\left\lbrack \mathbf{W}^{o\hspace{0pt}u\hspace{0pt}t}\tanh\left( \mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c}h_{t} \odot \mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}g_{u} + b \right) \right\rbrack
```
...
Higher-order interactions MI allows for second-order interactions between the elements of $`h_{t}`$ and $`g_{u}`$ which facilitates modeling of more complex dependencies between the acoustic and LM embeddings. Importantly, the shift to second order is achieved with no increase in the number of parameters or computational complexity. In theory, feedforward neural nets with sufficient capacity and training data are universal function approximators; therefore, a joint network with $`\left( h_{t},g_{u} \right)`$ inputs and multiple layers should be optimal. However the memory requirements are prohibitive because the input tensor size for such a network is $`N \times T \times U \times \left( {E + P} \right)`$ and the memory for the output tensors is $`N \times T \times U \times H \times L`$ where $`N`$ is the batchsize, $`H`$ is the number of hidden units and $`L`$ is the number of layers. Therefore, for pure training efficiency reasons, we are constrained to use shallow (single layer) joint networks, in which case the functional form of the layer becomes important.\
Generalizes additive integration Indeed, when used in conjunction with biases, the additive terms appear in the expansion

$`{\left( {{\mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c}\hspace{0pt}h_{t}} + b^{e\hspace{0pt}n\hspace{0pt}c}} \right) \odot \left( {{\mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}\hspace{0pt}g_{u}} + b^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}} \right)} = {{{\mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c}\hspace{0pt}h_{t}} \odot \mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}}\hspace{0pt}g_{u}}`$$`{+ {{b^{e\hspace{0pt}n\hspace{0pt}c} \odot \mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}}\hspace{0pt}g_{u}}} + {{b^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d} \odot \mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c}}\hspace{0pt}h_{t}} + {b^{e\hspace{0pt}n\hspace{0pt}c} \odot b^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}}`$\

Scaling and gating effect By multiplying $`{\overset{\sim}{h}}_{t}:={\mathbf{W}^{e\hspace{0pt}n\hspace{0pt}c}\hspace{0pt}h_{t}}`$ with $`{\overset{\sim}{g}}_{u}:={\mathbf{W}^{p\hspace{0pt}r\hspace{0pt}e\hspace{0pt}d}\hspace{0pt}g_{u}}`$, one embedding has a scaling effect on the other embedding. In particular, unlike for additive interactions, in MI the gradient with respect to one component is gated by the other component and vice versa. Concretely, let us denote by $`{\mathcal{L}\hspace{0pt}(\mathbf{x},\mathbf{y};\theta)} = {- {{\log p}\hspace{0pt}\left( \mathbf{y} \middle| {\mathbf{x};\theta} \right)}}`$ the NLL loss for one utterance for an RNN-T with parameters $`\theta`$. The partial derivatives of the loss with respect to $`{\overset{\sim}{h}}_{t}`$ and $`{\overset{\sim}{g}}_{u}`$ are:

$`\frac{\partial\mathcal{L}}{\partial{\overset{\sim}{h}}_{t}}`$$`= {{\overset{\sim}{g}}_{u} \odot \frac{\partial\mathcal{L}}{\partial\left( {{\overset{\sim}{h}}_{t} \odot {\overset{\sim}{g}}_{u}} \right)}}`$$`\frac{\partial\mathcal{L}}{\partial{\overset{\sim}{g}}_{u}}`$$`= {{\overset{\sim}{h}}_{t} \odot \frac{\partial\mathcal{L}}{\partial\left( {{\overset{\sim}{h}}_{t} \odot {\overset{\sim}{g}}_{u}} \right)}}`$
...
Speed and tempo perturbation \[19\] changes the rate of speech in the interval $`\lbrack 0.9,1.1\rbrack`$ with or without altering the pitch or timbre of the speaker. This technique generates additional replicas of the training data depending on the number of speed and tempo perturbation values.\
Sequence noise injection \[20\] adds, with a given probability, the downscaled spectra of randomly selected training utterances to the spectrum of the current training utterance. This technique does not increase the amount of training data per epoch.\
...
```math
\mathbf{y}^{\ast} = {\operatorname{argmax}\limits_{\mathbf{y} \in \mathcal{Y}^{\ast}}\left\{ {{{{\log p}\hspace{0pt}\left( \mathbf{y} \middle| \mathbf{x} \right)} - {\mu\hspace{0pt}{\log p^{s\hspace{0pt}r\hspace{0pt}c}}\hspace{0pt}(\mathbf{y})}} + {\lambda\hspace{0pt}{\log p^{e\hspace{0pt}x\hspace{0pt}t}}\hspace{0pt}(\mathbf{y})} + {\rho\hspace{0pt}|\mathbf{y}|}} \right\}}
```

\

where $`\mu,\lambda,\rho`$ are the weights corresponding to the source LM $`p^{s\hspace{0pt}r\hspace{0pt}c}`$, external LM $`p^{e\hspace{0pt}x\hspace{0pt}t}`$, and label length reward $`|\mathbf{y}|`$.
...
The first set of experiments was conducted on the Switchboard speech corpus which contains 300 hours of English telephone conversations between two strangers on a preassigned topic. The acoustic data segmentation and transcript preparation are done according to the Kaldi s5c recipe \[28\]. We report results on the commonly used Hub5 2000 Switchboard and CallHome test sets as well as Hub5 2001 and RT’03, which are processed according to the LDC segmentation and scored using Kaldi scoring for measuring WER.

We extract 40-dimensional speaker independent log-Mel filterbank features every 10 ms which are mean and variance normalized per conversation side. The features are augmented with $`\Delta`$ and $`\Delta\hspace{0pt}\Delta`$ coefficients and every two consecutive frames are stacked and every second frame is skipped resulting in 240-dimensional vectors extracted every 20 ms. These features are augmented with 100-dimensional i-vectors that are extracted using a 2048 40-dimensional diagonal covariance Gaussian mixture UBM trained on speaker independent PLP features transformed with LDA and a semi-tied covariance transform.

Speed and tempo perturbation is applied to each conversation side with values in $`\left\{ 0.9,1.1 \right\}`$ for both speed and tempo separately resulting in 4 additional training data replicas which, together with the original data, amounts to 1500 hours of training data per epoch. For sequence noise injection, we add, with probability 0.8, to the spectrum of each training utterance the spectrum of one random utterance of similar length scaled by a factor of 0.4. For SpecAugment we used the settings published in \[21\]. Lastly, in label switchout, for a sequence of length $`U`$, we first sample $`\hat{n} \in \left\{ 0,\ldots,U \right\}`$ with $`{p\hspace{0pt}\left( \hat{n} \right)} \propto e^{- {\hat{n}/\tau}}`$ and then we replace, with probability $`\left. \hat{n}/U \right.`$, the true characters with random characters for each position in the sequence. We set the temperature to $`\tau = 10`$ in our experiments.
...
|     |           |         |      |         |      |      |       |      |
| --- | --------- | ------- | ---- | ------- | ---- | ---- | ----- | ---- |
| ext | Model     | Hub5’00 |      | Hub5’01 |      |      | RT’03 |      |
| LM  |           | swb     | ch   | swb     | s2p3 | s2p4 | swb   | fsh  |
...
|     | Comb.     | 5.9     | 12.5 | 6.6     | 8.6  | 12.6 | 14.1  | 8.6  |
...
$`\mathbf{y}^{\ast} = \operatorname{argmax}\limits_{\mathbf{y} \in {{\mathcal{H}_{+}\hspace{0pt}{(\mathbf{x})}} \cup {\mathcal{H}_{\odot}\hspace{0pt}{(\mathbf{x})}}}}\{`$$`{\alpha\hspace{0pt}{\log p}\hspace{0pt}\left( \mathbf{y} \middle| {\mathbf{x};\theta_{+}} \right)} + {\beta\hspace{0pt}{\log p}\hspace{0pt}\left( \mathbf{y} \middle| {\mathbf{x};\theta_{\odot}} \right)}`$$`\left. - \mu\log p^{s\hspace{0pt}r\hspace{0pt}c}(\mathbf{y}) + \lambda\log p^{e\hspace{0pt}x\hspace{0pt}t}(\mathbf{y}) + \rho \middle| \mathbf{y} \middle| \right\}`$\

where $`{\mathcal{H}_{+}\hspace{0pt}(\mathbf{x})},{\mathcal{H}_{\odot}\hspace{0pt}(\mathbf{x})}`$ are the n-best hypotheses generated by the additive and multiplicative RNN-Ts. Concretely, we rescore the union of the top 32 hypotheses from each model with $`{\alpha = \beta = \mu = 0.5},{\lambda = 0.7}`$ and $`\rho = 0.2`$. The results from Table 3 show that multiplicative RNN-Ts are comparable on Hub5’00 and Hub5’01 but slightly worse on RT’03 and that model combination significantly improves the recognition performance across all test sets.
