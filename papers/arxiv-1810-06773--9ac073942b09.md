...
3.  3.  Fitness evaluation:   After the offspring population is generated, the fitness value for each individual in $`\Psi_{\mu + \lambda}^{(k)} = {\Psi_{\mu}^{(k)}\hspace{0pt}{\bigcup\Psi_{\lambda}^{(k)}}}`$ is evaluated.
4.  4.  $`m`$-elitist:   $`m`$ ($`1 \leq m \leq \mu`$) individuals with the best fitness are first selected from $`\Psi_{\mu + \lambda}^{(k)}`$. The rest $`\mu - m`$ individuals are then randomly selected from the other $`{\mu + \lambda} - m`$ candidates in $`\Psi_{\mu + \lambda}^{(k)}`$ to form the parent population $`\Psi_{\mu}^{({k + 1})}`$ of the next generation.
