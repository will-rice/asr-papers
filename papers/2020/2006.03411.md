...
As in \[7\], each context word, $`w_{i}`$, is first represented as a sequence of target sentence piece units, e.g. the word “Jarred” may be mapped to *\[Ja, r, re, d\]*. This sequence is then fed to an BLSTM, and the last state of the BLSTM is used as the embedding of the given context word (shown as $`h_{i}^{E\hspace{0pt}E}`$ in Figure 2).
...
We next compute a *context vector*, $`c_{u}`$, \[7, 13\] as the weighted sum of *word embedding* as:
