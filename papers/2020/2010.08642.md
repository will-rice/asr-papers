...
1.  1.
    Early Decoder Fusion (Early-DF): At each timestep, we concatenate $`\mathbf{v}`$ to the input embedding $`\mathbf{y}_{\mathbf{t}}`$, which is then projected down to the embedding dimension.

    $`\mathbf{y}_{\mathbf{t}}`$$`= {\mathbf{W}_{\mathbf{p}\mathbf{r}\mathbf{o}\mathbf{j}}\hspace{0pt}{\lbrack\mathbf{y}_{\mathbf{t}};\mathbf{v}\rbrack}}`$
2.  2.
    Weighted Early Decoder Fusion (Weighted-DF): We calculate a timestep-dependent weighted scalar between the input embedding $`\mathbf{y}_{\mathbf{t}}`$ and the embedded visual features $`\mathbf{v}`$ (Eqn. 7), which scales the contribution of the visual features in the concatenated input (Eqn. 8):
...
3.  3.
    Middle Decoder Fusion (Middle-DF): In this approach, fusion occurs between the GRU layers at $`\mathbf{z}_{\mathbf{t}}`$ (Eqn. 2), which is the input to the 2nd decoder layer:
...
## References
- Anastasopoulos et al. (2019) Antonios Anastasopoulos, Shankar Kumar, and Hank Liao. 2019. Neural language modeling with visual features. *arXiv preprint arXiv:1903.02930*.
...
- Barker et al. (2018) Jon Barker, Shinji Watanabe, Emmanuel Vincent, and Jan Trmal. 2018. The fifth ’chime’ speech separation and recognition challenge: Dataset, task and baselines. In *Interspeech*.
...
- Schuster and Paliwal (1997) Mike Schuster and Kuldip K Paliwal. 1997. Bidirectional recurrent neural networks. *IEEE transactions on Signal Processing*.
