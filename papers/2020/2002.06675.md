...
1.  1.  A word with a single letter is unchanged.
2.  2.  Two consecutive Cs and Vs are given a syllable boundary between them.\
                     R^(∗){CC, VV}R^(∗)$`\rightarrow`$ R^(∗){C-C, V-V}R^(∗)\
                                      (R $`≔`$ {C, V})
3.  3.  Put a syllable boundary after the segment-initial V if it is following by at least two phones.\
                                    VCR⁺$`\rightarrow`$ V-CR⁺
4.  4.  Put a syllable boundary after CV repeatedly from left to right until only CV or CVC is left.\
