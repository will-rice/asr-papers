...
Table 7: SNR₉₀ (in \[dB\]) of CV stimuli, average results across N\_(T) talkers, measured by human subjects (ground truth) and ASR. The PESQ score is calulated at ASR SNR₉₀. Bias (in \[dB\]) and variance ($`\sigma^{2}`$) of ASR SNR₉₀ estimation (in \[dB²\]), as well as our model's test MSE (in \[dB²\]) are provided.
