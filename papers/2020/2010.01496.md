...
1.  1.
    Choose a task and a dataset $`\mathcal{D}`$.
2.  2.
    Choose a selector-predictor model $`m`$.
3.  3.
    Train $`m`$ on $`\mathcal{D}`$.
4.  4.
    Using the trained and fixed model $`m`$, prune the dataset $`\mathcal{D}`$ to $`\mathcal{G}^{m} \subseteq \mathcal{D}`$ by first eliminating the instances x that do not satisfy Equation 4.2, and then by further eliminating the instances for which $`\text{SIR}_{\text{x}}^{m} = \varnothing`$ according to Equation 4.1.
5.  5.
    Using the metrics defined above, test an explainer on $`m`$ applied to instances in $`\mathcal{G}^{m}`$.
...
1.  1.
    the selector (called generator in their paper) takes as input a piece of text x and, for each feature (token in our case) $`x_{i} \in \text{x}`$, outputs $`p_{i} \in {\lbrack 0,1\rbrack}`$. Then, $`p_{i}`$ is used as the parameter of a Bernoulli distribution modelling the probability that the token $`x_{i}`$ is selected, and
2.  2.
    the predictor (called encoder in their paper) takes as input $`\text{S}_{\text{x}}^{m} = {\{{x_{i} \sim {\text{Bernoulli}\hspace{0pt}{(p_{i})}}}\}}`$ (at test time $`\text{S}_{\text{x}}^{m} = \left. \{ x_{i} \middle| {p_{i} \geq 0.5}\} \right.`$) and returns the prediction as a real number in $`\lbrack 0,1\rbrack`$.
...
- •
  At step (B), the model has to generate a exact target sequence, since the goal is to generate the *exact* explanation that was identified at step (A) as inconsistent with the explanation $`\mathbf{e}_{m}\hspace{0pt}{(\text{x})}`$.
- •
...
For the above reasons, given an instance x, I differentiate between parts of the instance that will remain fixed in InconsistVerif (referred to as *context parts* and denoted as $`\mathbf{x}_{c}`$) and parts of the instance that InconsistVerif will vary in order to obtain inconsistencies (referred to as *variable parts* and denoted as $`\mathbf{x}_{v}`$). Hence, $`\text{x} = {(\mathbf{x}_{c},\mathbf{x}_{v})}`$. In the VQA example above, $`\mathbf{x}_{c}`$ would be the image, and $`\mathbf{x}_{v}`$ would be the question.
...
1.  1.
    Reverse the explanation generator module of model $`m`$ by training a RevExpl model to map from the generated explanation and the context part of the input to the variable part of the input, i.e., $`{\text{RevExpl}\hspace{0pt}{(\mathbf{x}_{c},{\mathbf{e}_{m}\hspace{0pt}{(\text{x})}})}} = \mathbf{x}_{v}`$.
2.  2.
    For each explanation $`\mathbf{e} = {\mathbf{e}_{m}\hspace{0pt}{(\text{x})}}`$:

    1.  (a)
        Create a list of statements that are inconsistent with $`\mathbf{e}`$, referred as $`\mathcal{I}_{\mathbf{e}}`$.
    2.  (b)
        Query RevExpl on each $`\hat{\mathbf{e}} \in \mathcal{I}_{\mathbf{e}}`$ and the context $`\mathbf{x}_{c}`$. Get the new variable part $`{\hat{\mathbf{x}}}_{v} = {\text{RevExpl}\hspace{0pt}{(\mathbf{x}_{c},\hat{\mathbf{e}})}}`$ of a *reverse input* $`\hat{\mathbf{x}} = {(\mathbf{x}_{c},{\hat{\mathbf{x}}}_{v})}`$, which *may* cause $`m`$ to produce inconsistent explanations.
    3.  (c)
        Query $`m`$ on each reverse input to get a *reverse explanation* $`\mathbf{e}_{m}\hspace{0pt}{(\hat{\mathbf{x}})}`$.
    4.  (d)
