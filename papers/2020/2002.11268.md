---
arxiv_id: '2002.11268'
title: A Density Ratio Approach to Language Model Fusion in End-To-End Automatic Speech
  Recognition
authors:
- Erik McDermott
- Hasim Sak
...
1.  1.
    The source domain $`\psi`$ has some true joint distribution $`P_{\psi}\hspace{0pt}{(W,X)}`$ over text and audio;
2.  2.
    The target domain $`\tau`$ has some other true joint distribution $`P_{\tau}\hspace{0pt}{(W,X)}`$;
3.  3.
    A source domain end-to-end model (e.g. RNN-T) captures $`P_{\psi}\hspace{0pt}{(\left. W \middle| X \right.)}`$ reasonably well;
4.  4.
    Separately trained LMs (e.g. RNN-LMs) capture $`P_{\psi}\hspace{0pt}{(W)}`$ and $`P_{\tau}\hspace{0pt}{(W)}`$ reasonably well;
5.  5.
    $`p_{\psi}\hspace{0pt}{(\left. X \middle| W \right.)}`$ is roughly equal to $`p_{\tau}\hspace{0pt}{(\left. X \middle| W \right.)}`$, i.e. the two domains are acoustically consistent; and
6.  6.
    The target domain posterior, $`P_{\tau}\hspace{0pt}{(\left. W \middle| X \right.)}`$, is unknown.
