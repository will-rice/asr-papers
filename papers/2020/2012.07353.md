---
arxiv_id: '2012.07353'
title: 'REDAT: Accent-Invariant Representation for End-to-End ASR by Domain Adversarial
...
We use *softmax* to normalize the final outputs such that the probability $`C_{i}\hspace{0pt}{(z)}`$ of an input utterance belonging to the $`i`$-th accent must satisfy,
...
Table 1: Normalized WERs¹ on 23K hours of en-X data. *AS* or *AI* denotes an accent-specific or accent-invariant model; *native* or *non-native* denotes native or non-native speakers on test sets; *unsup8* or *unsup20* denotes *reDAT* with 8 or 20 unsupervised clusters; *soft* denotes *reDAT* with soft labels.
