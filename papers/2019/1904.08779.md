...
2.  2.
    Frequency masking is applied so that $`f`$ consecutive mel frequency channels $`\left\lbrack f_{0},{f_{0} + f} \right)`$ are masked, where $`f`$ is first chosen from a uniform distribution from 0 to the frequency mask parameter $`F`$, and $`f_{0}`$ is chosen from $`\left\lbrack 0,{\nu - f} \right)`$. $`\nu`$ is the number of mel frequency channels.
3.  3.
    Time masking is applied so that $`t`$ consecutive time steps $`\left\lbrack t_{0},{t_{0} + t} \right)`$ are masked, where $`t`$ is first chosen from a uniform distribution from 0 to the time mask parameter $`T`$, and $`t_{0}`$ is chosen from $`\left\lbrack 0,{\tau - t} \right)`$.
...
1.  1.
    B(asic): $`\left( s_{r},s_{\text{noise}},s_{i},s_{f} \right)`$ = (0.5k, 10k, 20k, 80k)
2.  2.
    D(ouble): $`\left( s_{r},s_{\text{noise}},s_{i},s_{f} \right)`$ = (1k, 20k, 40k, 160k)
...
1.  3.
    L(ong): $`\left( s_{r},s_{\text{noise}},s_{i},s_{f} \right)`$ = (1k, 20k, 140k, 320k)
...
- \[21\] Ç. Gülçehre, O. Firat, K. Xu, K. Cho, L. Barrault, H. Lin, F. Bougares, H. Schwenk, and Y. Bengio, “On using monolingual corpora in neural machine translation,” in *arxiv*, 2015.
