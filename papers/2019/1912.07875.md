...
1.  1.
    The raw waveform $`w`$ goes through a convolutional network $`g_{c}`$, resulting in a feature sequence $`{(\mathbf{x}_{t})}_{t \in {1\hspace{0pt}\ldots\hspace{0pt}T}}`$.
2.  2.
    Then we form the current phoneme representation $`\mathbf{z}_{t}`$ by applying a recurrent network $`g_{a\hspace{0pt}r}`$ to $`\mathbf{x}_{t}`$.
3.  3.
    Finally, we predict $`(\mathbf{x}_{t_{0} + 1},\ldots,\mathbf{x}_{t_{0} + k})`$ from $`{(\mathbf{z}_{t})}_{t \leq t_{0}}`$ using a prediction network $`g_{p}`$.
