...
arxiv_id: "2202.13097"
title:
  Language-Independent Speaker Anonymization Approach using Self-Supervised Pre-Trained
  Models
authors:
  - Xiaoxiao Miao
  - Xin Wang
...
Table 2: EER (%) on English data (development set of VPC 2020) evaluated by _$`A\hspace{0pt}S\hspace{0pt}V_{}^{}`$\_. The EER results of the OO condition for Libri-female, Libri-male, VCTK-diff-female, and VCTK-diff-male are 8.66%, 1.24%, 2.86%, and 1.43%, respectively. Higher EER indicates better privacy.
...
Table 3: EER (%) on English data (test set of VPC 2020) evaluated by _$`A\hspace{0pt}S\hspace{0pt}V_{}^{}`$\_. The EER results of the OO condition for Libri-female, Libri-male, VCTK-diff-female, and VCTK-diff-male are 7.66%, 1.11%, 4.88%, and 2.06%, respectively. Higher EER indicates better privacy.
...
The evaluation plan also uses the word error rate (WER) as a utility metric to measure how well the speech content is preserved after anonymization. The WER was computed using an ASR model (_$`A\hspace{0pt}S\hspace{0pt}R_{}`$_). An ideal anonymization system should not increase the WER of test trials after anonymization. Note that both _$`A\hspace{0pt}S\hspace{0pt}R*{}`$* and _$`A\hspace{0pt}S\hspace{0pt}V_{}`$_ are independent from the anonymization system. For the experiments on different languages, we use different _$`A\hspace{0pt}S\hspace{0pt}R*{}`$* and _$`A\hspace{0pt}S\hspace{0pt}V_{}`$\_ to evaluate the anonymization systems more accurately. Details are explained in the following sections.
...
Table 4: WER (%) on English data evaluated by _$`A\hspace{0pt}S\hspace{0pt}R_{}^{}`$\_. Lower WER indicates better utility.
...
Tables 2 and 3 show the EER results of various speaker anonymization systems, measured by _$`A\hspace{0pt}S\hspace{0pt}V_{}^{}`$_ on the VPC 2020 development and test sets, respectively. Table 4 shows their WER results measured by _$`A\hspace{0pt}S\hspace{0pt}R*{}^{}`$*. VPC 2020 baseline B2 was not included in the objective evaluation as its objective results are worse than B1\[5\]. Here is the point-by-point summary:
...
The evaluation was conducted on a test set sampled from a multi-speaker Mandarin speech corpus called AISHELL-3\[24\]. The test set contains 4,267 utterances from 44 speakers. We split them into test trial (88 utterances) and enrollment (4179 utterances) subsets, which are used to produce 10,120 enrollment-test trials for the ASV evaluation. The ASV evaluation model _$`A\hspace{0pt}S\hspace{0pt}V_{}^{}`$_ was a F-ECAPA trained on Mandarin datasets called CN-Celeb-1 & 2 \[47, 48\]. _$`A\hspace{0pt}S\hspace{0pt}V*{}^{}`$* was configured in the same way as Section 4.1 described. The ASR evaluation model _$`A\hspace{0pt}S\hspace{0pt}R_{}^{}`$\_ was a publicly available ASR Transformer \[43\] trained on the 150-hour Mandarin ASR dataset AISHELL-1 \[49\]. Note that the WER was replaced with character error rate (CER) in Mandarin ASR.
...
Table 5: EER (%) on Mandarin data evaluated by _$`A\hspace{0pt}S\hspace{0pt}V_{}^{}`$\_. Higher EER indicates better privacy.
...
Table 6: CER (%) on Mandarin data evaluated by _$`A\hspace{0pt}S\hspace{0pt}R_{}^{}`$\_. Lower CER indicates better utility.
